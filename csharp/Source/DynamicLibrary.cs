...
namespace pw
{
    class DynamicLibrary
    {
        ...
        /// <summary>
        /// Creates an instance of a DynamicLibrary without loading a module into it.
        /// </summary>
        ...
        /// <exception cref="System.ArgumentNullException">Thrown when the given path is null or empty.</exception>
        /// <exception cref="System.InvalidOperationException">Thrown when there's already a module loaded by this instance.</exception>
        ...
        {
            if (string.IsNullOrEmpty(_sPath)) throw new ArgumentNullException("Input parameter cannot be null or empty string");
            if (m_ptrModule != IntPtr.Zero) throw new InvalidOperationException("There's still a module loaded. Call Unload before loading another module.");
        ...
        {
            if (m_ptrModule == null) throw new InvalidOperationException("Module hasn't been loaded yet.");
        ...
        /// </summary>
        /// <param name="_iID">The identifier of the string to be loaded.</param>
        /// <param name="_sFallback">A fallback string, that is returned, if the string couldn't be found or no module has been loaded.</param>
        /// <param name="_iMaxStringLength">The maximum string length. The string is truncated and null-terminated if it is longer than the number of characters specified.</param>
        /// <returns>If the function succeeds, it returns the read string. It returns the string specified by the _sFallback-parameter otherwise.</returns>
        public string GetString(uint _iID, string _sFallback, int _iMaxStringLength = 256)
        {
            try
            {
                string sReturn = GetString(_iID, _iMaxStringLength);
                if (!string.IsNullOrEmpty(sReturn))
                {
                    return sReturn;
                }

                throw new InvalidOperationException("String could not be read from the module.");
            }
            catch
            {
                return _sFallback;
